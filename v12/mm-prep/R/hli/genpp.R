...
main <- function(v2pepwrgen, femleg, lgbicam, utable, VARNAME) {
    ...
    nonc.df <-
        ...
        interpolate_vdem_col("v2lgbicam", utable) %>%
        mutate(v2lgfemleg =
...
if (no_test()) {
    ...
    # Run
    collectedInputs <- named_list(v2pepwrgen, femleg, lgbicam, utable, VARNAME)
    setNames(list(do.call(main, collectedInputs)), VARNAME) %>%
        write_file(., OUTFILE, dir_create = TRUE)

    } else {
        testthat::test_file("~/proj/mm-prep/tests/hli/test_genpp.R")
}
