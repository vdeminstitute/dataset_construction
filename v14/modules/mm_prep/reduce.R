...
# measurement model. 
#
# A reduction *periods* are sequences of chronological across only consecutive
...
#
# This applies to code only, not confidences.
#
# For confidences, we first calculate the day-weighted confidence per coder
# within a coding regime before reducing them to one observation per regime.
# Effectively, this means that we reduce confidences on the basis of how we reduced
# code.
# 
# ==========================================================================
        ...
        lapply(X=_, function(subM) {
            ...
            for (i in 2:nrow(subM)) {
                ...
                if(compare_rows(vtrack, vcurrent)) {
                    ...
                    # if there is new information [e.g. c(1,1), c(1,2)], do:
...
date_to_weights_across_years <- function(v) {
    ...
    # Compute the unique number of years as a scalar
