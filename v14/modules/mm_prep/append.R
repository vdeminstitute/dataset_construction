...
create_historical_new_coder_matrices <- function(wdata, coder_table) {
    ...
    ml <- lapply(ll, function(m, vcd) {
        ...
        # Define logical matrices for historical or new coders, that has TRUE
        # whenever:
        # 1. A coder is flagged as either historical or new, and:
        # 2: The score is non-missing
        # All other values are set to FALSE.
        # These matrices act as input for the make_prior function to identify
        # offsets for priors.
...
make_priors <- function(input.data, normal_vignettes.b, lateral_vignettes.b, TASK_NAME) {
    ...
    if (substring(TASK_NAME, 1, 2) == "v2") {
        ...
        if (any(historical)) {
            ...
            # Apply our offsets from successor states to extinct ones 
