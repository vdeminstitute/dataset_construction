...
#
# Depending on the target to create, we process them differently.
#
...
# -- use text_answer from v2reginfo/v2reginfo (after const_hist_merged).
# -- Following the introduction of January 1 as default dates, there can arise missing values in the value column.
create_duration_var <- function(df) {
	...
	# We need to the text_answer column to have a "date-like" format.
    # The regime name is followed by the string: dd/mm/yyyy - dd/mm/yyyy or E.
    stopifnot(`text_col lacks a date-like structure` = grepl(
        pattern = "\\d{2}/\\d{2}/\\d{4}\\s[-–—−―]\\s\\d{2}/\\d{2}/\\d{4}|E",
        x = df[["text_answer"]]))

    # match dates, split them // match each date separately and save in separate column
    # calculate number of days starting from the regime beginning 
...
# -- regidnr is the combination of country_id and a two digit counter that increments at regime change:
# -- for example: country_id is 3 and the first regime is 01 -> regidnr is 301, next is 302.
...
# -- only one "top" function for two variables
