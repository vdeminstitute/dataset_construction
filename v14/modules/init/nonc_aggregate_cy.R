...
# (day-weighted mean).
# Data must be already in wide format!
# ==========================================================================
...
find_aggregation_method <- function(qtable, TASK_NAME) {
    ...
    lastVars <- c(
        "v2exparhos", "v2expothog", "v2exnamhos", "v2exnamhog", "v2exhoshog", "v3exnamhos", "v3exnamhog"
    )

    hardcodedMaxVars <- c(
            "v3eldirelc", "v3eldireuc", "v3eldirepr",
...
aggr_ratio <- function(df) {
    # Script is ran in parallel, so mc.cores for each variable is 1.
