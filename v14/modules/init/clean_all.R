...
clean_duplicates <- function(df) {
    ...
    # -- D is here because we have indices in Postgres with coder_id
...
# If (2) |Exists| / |Ratings| >= 0.5 \for all from (1) -> keep
# NOTE: for (2), when two coders are pitched against each other
# for a given c-y, we remove the ratings. 
mecenefi_cleaning <- function(df, TASK_NAME) {
    ...
    # This is what we want to remove, so do not negate
    # it for clean_observations()
