...
# -- we save stata versions to a separate directory for post-processing reasons (stata_labels module)
...
fu <- function(v) {
    ...
    lapply(c("cd", "cy"), function(lev) {
        tryCatch({
            ...
            print(paste0("error when extracting objects from task file: ", v))
            return(NULL)
        }, warning = function(war) {
            print(paste0("warning when extracting objects from task file: ", v))
            return(NULL)
        })
    })
    out
...
llf <- mclapply(deps, fu, mc.cores = 9, mc.preschedule = FALSE)
...
subset(tasks, task_id %in% bad_deps, select = c(
...
# df_cy <- read_file(file.path(ROOT, "llf", "df_cy.rds"))
# df_cd <- read_file(file.path(ROOT, "llf", "df_cd.rds"))
# ------------------------------------------------------------------------------
...
df_cy <- merge(
    x = df_cy,
    y = read_file(file.path(ROOT, "party_systems", "vparty_index.rds")),
    by = c("country_id", "year"),
    all = TRUE)

# -- add country-date here
df_cd <- merge(
    x = df_cd,
    y = read_file(file.path(ROOT, "party_systems", "vparty_index_cd.rds")),
    by = c("country_id", "historical_date"),
    all = TRUE)

stopifnot(!anyNA(df_cy$country_text_id))
stopifnot(!anyNA(df_cd$country_text_id))
stopifnot(!any(df_cd[, grep("v2xpas", value = T, x = colnames(df_cd))] == -11111, na.rm = TRUE))
stopifnot(!any(df_cy[, grep("v2xpas", value = T, x = colnames(df_cy))] == -11111, na.rm = TRUE))
...
df_cd <- left_join(df_cd, select(e_data, country_id, year, COWcode),
    by = c("country_id", "year"))
...
if (interactive()) {
    ...
    old_cd <- read_file("~/data/ds_construction/v13/dataset/V-Dem-CD/V-Dem-CD-v13.rds")
    # what is in new but not in old?
