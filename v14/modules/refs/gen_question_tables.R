...
# Many of the different columns indicate what sections of the 
# data pipeline need to be run for each variable. 
# This table is being used in almost every script and it is also 
# used in constructing the data pipeline task list.
# The table includes meta data with information on each column 
# (callable with meta())
...
merge_question_codebook <- function(question, codebook) {
    ...
    # Update is_party column to also include the vignettes
...
# mostly important for populate make
# do note that some entries are hardcoded
identify_index_types <- function(qtable) {
	# Determine mm-prep variables (includes percentage that don't go through mm)
	qtable %>% 
		...
		mutate(mm = grepl("Bayesian item response theory",
                              crosscoder_aggregation,
                              fixed = T)) %>%
		# Do not run through mm when cont-hist variables are merged
		mutate(mm = ifelse(hist_merge_no_conflict, F, mm)) %>%
		# Determine variables that have NA as historical_date
		mutate(NA_historical_date = grepl("D",
                                          question_type,
                                          fixed = TRUE)) %>%
		# variables that are binary indices:
...
identify_recoded_variables <- function(qtable, dich_df) {
	...
	# c("v2eldonate", "v2psprbrch","v2elpeace", "v2exdfcbhs", "v2peasbsoc", "v2lgcrrpt")
	...
	# Set _rec variables to dichotomized = FALSE
	qtable$dichotomized[grepl("_rec$", qtable$name)] <- FALSE

	# Adjust v2jupack category for recoding
	# See codebook entry for v2jupack
...
recode_category_to_binary <- function(qtable) {
	...
	# Create identifier: 'recode_category_to_binary' says which answer category
    # that should be turned into a new variable
	qtable %<>%
...
adjust_cb_responses <- function(qtable) {
    ...
    # If a variable exists both as v2 and v3, fill in cb_responses from the v2
    # entry for v3 IF it is missing for v3
	qtable %<>%
	    group_by(parent_tag) %>%
    ...
    # cb_resonses are hardcoded to include the answer category that is stripped
    # away when the bin version is created. We need to preserve the original
    # responses until the first two cleaning scripts in the pipeline have been
    # executed.
	qtable %<>%
...
add_survey <- function(qtable, survey_tbl, question_queue_tbl) {
    ...
    # hard coded
    # backfill_question, in combination with backfill_coder from coder table in
    # vdem_data, identifies which question-coder combination that are
    # sequentially coded from 1900 and forwards, 
	qtable %<>%
...
correct_downloaded_tables <- function(qtable) {
	...
	# Do not get class D or E from our data
	qtable$rating[qtable$class %in% c("D", "E")] <- FALSE
	qtable$a_data[qtable$class %in% c("D", "E")] <- FALSE
	# Except for v2xdd_ variables and elecreg which has class D,
	# but is stored in the rating table
...
choice %<>%
    ...
    # add fix for COVID-19 variables
    bind_rows(
