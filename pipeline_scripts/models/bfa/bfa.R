...
for (p in c("xi", "gamma...1", "gamma...2", "omega")) {
    stat <- mean(g[grepl(p, rownames(g)), 1] > 1.1)
    info("Gelman-Rubin Rhat for " %^% p %^% ": " %^% stat)

    if (stat > 0.05) {
        warn("Convergence check failed for " %^% p)
        info("Proportion above 1.1: " %^% stat)
    }
}
...
# gamma[1] is the intercept, gamma[2] the slope, omega is uniqueness
combined.posterior <- lapply(posteriors, function(o)  {
    m <- as.matrix(o)
...
output$factors <- res
# use cat(output$model_code) to see JAGS code conveniently
