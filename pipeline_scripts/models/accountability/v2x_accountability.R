...
write_file(hmodel1, file.path("out",  sprintf("%s_%s_%s", NAME, Sys.getenv("SLURM_JOB_ID"), "hmodel1.rds")))
...
info(sprintf("%s chains of JAGS models finished", CHAINS))
write_file(hmodel2, file.path("out",  sprintf("%s_%s_%s", NAME, Sys.getenv("SLURM_JOB_ID"), "hmodel2.rds")))
...
xi <- idbayes(combine.mcmc(posteriorDraws[, 112:(111 + nrow(acc))]))
